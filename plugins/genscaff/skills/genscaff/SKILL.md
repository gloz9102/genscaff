...
5. Preserve architecture and implement the selected direction, primary flow, and relevant states. For interactive work, apply `references/product-specificity-and-action-gate.md` while wiring controls, before the first render. Follow the routed craft requirements without weakening them for exploration.
...
## UI defaults for every profile

- Do not compress all content into one screen or viewport. Show what the current task needs and allow scrolling, separate sections, or progressive disclosure for secondary detail while preserving required information and action access. Dashboards, back offices, other information-intensive surfaces, and explicit user requests may justify higher density.
- Whitespace is acceptable. Do not add content, enlarge elements, or tighten spacing merely to fill the screen; leave comfortable space unless usability evidence or a user request calls for a change.
- When text changes context or topic, start a new rendered line or paragraph. Keep unrelated messages on separate lines; use semantic text blocks or intentional line breaks rather than source-code newlines that HTML collapses. Preserve natural responsive wrapping within each thought.
- Give direct-action controls visible hover feedback on hover-capable devices and immediate press/click feedback by default. Preserve visible keyboard focus and equivalent touch/keyboard activation feedback; respect reduced motion. Reuse established states first. If the appropriate reaction remains unclear from the action and project conventions, ask the user about that control while continuing independent work.
- Choose a suitable primary accent from the existing design system or product context without requiring a color choice up front. Centralize the accent, its foreground, and related interaction states in existing theme tokens or a minimal shared token layer so a requested color change updates all consumers without per-component edits. Recheck contrast when changing it; a theme editor is not required.

## Product expression principles

Apply these six principles within the selected profile; Quick does not acquire Standard's documentation or audit requirements.

1. Give each section one core message or decision. Keep related comparison data together when the task requires it; identify why the next section follows instead of filling a preset section recipe.
2. Use whitespace to direct attention and separate topics. Neither fill space for its own sake nor hide missing required content behind minimalism.
3. Make visual material explain the claim: show the product, a relevant usage scene, a process, or an observable result. Use disclosed concept imagery when appropriate; never invent evidence or copy a reference brand's assets or exact composition.
4. Separate immediate control feedback from content choreography. Choose role-appropriate color, underline, border, or internal motion rather than universal scaling; keep layout, hit areas, and accessible names stable. Hover, press, focus, and touch alternatives must remain usable without waiting for scroll reveals or media. Revealed content needs click/keyboard access and complete submenu, back/close, and focus-return paths where applicable. Explanatory motion needs a reduced-motion/static path and must not trap scrolling or block reading.
5. Match expressive intensity to the product and audience. Quiet, playful, and product-led surfaces need not share a palette, geometry, or display scale. Reuse existing tokens first; assess background, color, rounding, and motion by hierarchy, readability, and task fit rather than numeric style quotas.
6. Recompose mobile content around message and action priority rather than shrinking desktop layouts; keep visual, reading, and focus order coherent. Choose horizontal or stacked actions from label length, available width, and target size. Preserve meaningful media crops and verify Korean line breaks in rendered desktop/mobile layouts. Separate changed topics with semantic blocks; preserve phrase/word boundaries within a thought and avoid desktop-only hard breaks that fragment mobile headings or controls. Allow safe wrapping of long untrusted strings. Distinguish viewport resizing, touch emulation, and real-device evidence; width alone does not establish input capabilities.
